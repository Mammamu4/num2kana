...
import { getMyriad, getNumberOfMyriads, myriadSetToKana, } from "../src/utils.js";
import { numToKana, numToHiragana, numToKatakana, numToRomaji, numToKanji, } from "../src/index.js";
...
test("myriadSetToKana", async (t) => {
    const hiraganaOptions = { type: "hiragana" };
    const katakanOptions = { type: "katakana" };
    const romajiOptions = {
        type: "romaji",
        spaceRomaji: false,
    };
    const romajiSpaceOptions = {
        type: "romaji",
        spaceRomaji: true,
    };
    await t.test("converts simple numbers to hiragana correctly", () => {
        ...
        assert.strictEqual(myriadSetToKana(123, hiraganaOptions), "ひゃくにじゅうさん");
        assert.strictEqual(myriadSetToKana(1234, hiraganaOptions), "せんにひゃくさんじゅうよん");
    });
    ...
    await t.test("handles spacing in romaji correctly", () => {
        assert.strictEqual(myriadSetToKana(123, romajiSpaceOptions), "hyaku nijuu san");
...
test("numToKana", async (t) => {
    ...
    await t.test("converts single digit numbers correctly with default kanji", () => {
    ...
    await t.test("handles spacing in romaji correctly", () => {
        assert.strictEqual(numToKana(12345, { type: "romaji", spaceRomaji: true }), "ichiman nisen sanbyaku yonjuu go");
        assert.strictEqual(numToKana(12345, { type: "romaji", spaceRomaji: false }), "ichimannisensanbyakuyonjuugo");
    });
});
...
test("helper functions", async (t) => {
    ...
    await t.test("numToHiragana converts correctly", () => {
        ...
        assert.strictEqual(numToHiragana(1234567), "ひゃくにじゅうさんまんよんせんごひゃくろくじゅうなな");
    });
    await t.test("numToKatakana converts correctly", () => {
        assert.strictEqual(numToKatakana(123), "ヒャクニジュウサン");
        assert.strictEqual(numToKatakana(1234567), "ヒャクニジュウサンマンヨンセンゴヒャクロクジュウナナ");
    });
    await t.test("numToRomaji converts correctly", () => {
        assert.strictEqual(numToRomaji(123), "hyaku nijuu san");
        assert.strictEqual(numToRomaji(123, { spaceRomaji: false }), "hyakunijuusan");
        assert.strictEqual(numToRomaji(1234567), "hyaku nijuu sanman yonsen gohyaku rokujuu nana");
    });
});
